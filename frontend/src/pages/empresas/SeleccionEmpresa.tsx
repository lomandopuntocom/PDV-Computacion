  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'white', borderRadius: 16, padding: 40, width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <h1 style={{ margin: '0 0 8px', fontSize: 24, color: '#1e293b' }}>Sistema de Inventario</h1>
        <p style={{ margin: '0 0 32px', color: '#64748b', fontSize: 14 }}>Selecciona una empresa para continuar</p>

        {loading ? (
          <p style={{ color: '#94a3b8', textAlign: 'center' }}>Cargando empresas...</p>
        ) : empresas.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {empresas.map(empresa => (
              ...
              >
                ...
                {empresa.ruc && <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>RUC: {empresa.ruc}</div>}
              </button>
            ))}
          </div>
        )}
