            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  {['Código', 'Nombre', 'Categoría', 'Unidad', 'Precio', 'Estación', 'Estado', 'Acciones'].map(col => (
              ...
              <tbody>
                {productosFiltrados.map((p, i) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: 14, color: '#64748b' }}>{categorias.find(c => c.id === p.categoriaId)?.nombre ?? p.categoria}</td>
                    <td style={{ padding: '12px 16px', fontSize: 14, color: '#64748b' }}>{unidades.find(u => u.id === p.unidadId)?.nombre ?? p.unidad}</td>
                    <td style={{ padding: '12px 16px', fontSize: 14, fontWeight: 'bold', color: '#1e293b' }}>Bs. {p.precio.toFixed(2)}</td>
